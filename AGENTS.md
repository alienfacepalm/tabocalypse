...
| **Product**           | No publisher backend; no publisher API keys; BYO AI only; declarative plugins only (no user JS from imports); donate/purchase/ideas are link-out only (no in-extension checkout; offline signature verification of purchased tokens or signed JSON is allowed); **no ads anywhere** (extension, homepage, first- or third-party packs/plugins — creators may sell their own signed JSON but may not carry advertising); no scheduled delivery obligations — see [doc/PLAN/MONETIZATION.md](doc/PLAN/MONETIZATION.md). The HUD search **assist** control opens third-party AI/search URLs in a new tab only — no publisher API keys or backend for that handoff, and no inlined answers on the new tab page.                                                                |
...
| **Verify**            | **`pnpm check`** runs format check, lint (zero warnings), tests, SDK + extension `tsc`. **Git:** `.husky/pre-commit` runs **lint-staged** on staged files. For **meaningful** changes, also run extension **`pnpm build`** when packaging is affected, then **commit** and **push** — see [`.cursor/rules/validate-commit-push.mdc`](.cursor/rules/validate-commit-push.mdc). Before commit: keep **[`doc/CHANGELOG.md`](doc/CHANGELOG.md) [Unreleased]** current for user-facing work; for major / roadmap work update **[`doc/PLAN/ROADMAP-PM-BOARD.md`](doc/PLAN/ROADMAP-PM-BOARD.md)** + **`pnpm pm:sync`** — see [update-docs-before-commit.mdc](.cursor/rules/update-docs-before-commit.mdc) and [tabocalypse-pm-board.mdc](.cursor/rules/tabocalypse-pm-board.mdc). |
