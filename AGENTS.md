...
| **UI styling**        | **Tailwind CSS** for layout and visuals; do not add parallel CSS systems. Legacy `style.css` may remain until migrated — see `.cursor/rules/project-conventions.mdc`.                                                                                                     |
| **TypeScript**        | No `any`. Match naming style in the folder you edit (`lib/` uses unprefixed interfaces like `Settings`).                                                                                                                                                                  |
...
| **Verify**            | **`pnpm run check`** runs format check, lint (zero warnings), tests, SDK + extension `tsc`. **Git:** `.husky/pre-commit` runs **lint-staged** on staged files.                                                                                                            |

**Build (from repo root):** `pnpm run build` (Chrome MV3), `pnpm run build:firefox`, `pnpm run dev`. Extension output: `apps/extension/.output/`.
