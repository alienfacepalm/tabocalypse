...
import { runOneShotWeatherGeolocation } from "./weather-geolocation";

describe("runOneShotWeatherGeolocation", () => {
  it("reports unavailable when geolocation is missing", async () => {
    const onOutcome = vi.fn();
    runOneShotWeatherGeolocation(undefined, onOutcome);
  ...
  it("delegates to getCurrentPosition and maps success", () => {
    ...
    const onOutcome = vi.fn();
    runOneShotWeatherGeolocation(geo, onOutcome);
