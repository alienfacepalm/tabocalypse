...
- **BYO AI** sends chat requests to the **base URL you configure** (OpenAI or Gemini presets, any other HTTPS OpenAI-compatible endpoint, or a local server on `localhost`) using **your** API key when you click “Test chat completion” in Settings or send a message in the **AI chat** widget (if enabled). The extension asks for permission to that host only when you save the base URL. Conversation text in the widget stays in memory for that tab session until you reload. You are subject to that provider’s privacy policy and billing.
...
**Planned, not yet shipped:** if optional paid content (a premium pack, a theme suite, or a Pro license) is offered in a future version, it will be bought from a third-party merchant and delivered to you as a file or text token. The extension will verify its signature **offline** using a public key built into the extension. No purchase, license, or account information will be sent to AlienFacepalm, and no network request will be made to validate it. A license token you paste in may sync via your browser's extension storage sync like other preferences; the default Settings export will omit it. The current release contains no license or signature code (see `doc/PLAN/MONETIZATION.md`).
