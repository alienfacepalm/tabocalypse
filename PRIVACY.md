...
- **CoinGecko** (`api.coingecko.com`, `assets.coingecko.com`) when the Crypto prices widget is enabled — public USD spot and chart samples for the coins on your watchlist (default BTC and ETH), plus coin logos from CoinGecko’s CDN (no Tabocalypse account; no API key shipped by the publisher).
